...
class OpenFoodFactsAPI:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt == self.max_retries:
                    logger.error("Request failed after %d attempts: %s", self.max_retries + 1, e)
                    break
                ...
                logger.debug(
                    ...
                    attempt, self.base_backoff_delay, backoff_delay, delay
                )
                logger.warning(
    ...
    def paginate_products(self, params: dict, max_pages: int = None) -> tuple[Iterator[dict], int]:
        ...
        def product_iterator():
            ...
            while True:
                logger.debug(
                    "Processing page %d with %d items per page",
                    page, self.page_size
                )
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        page, len(page_products), total_products_yielded, total_count
                    )
